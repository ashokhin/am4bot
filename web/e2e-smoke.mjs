...
try {
  ...
  await page.fill('[role=dialog] input#new-user-login', 'friend1')
  await page.fill('[role=dialog] input[type=password]', 'friendpass123')
  await page.click('[role=dialog] button[type=submit]')
  await page.waitForSelector('td:has-text("friend1")')
  assert(await page.isVisible('td:has-text("friend1")'), 'new user appears in the table')
  ...
  const friendRow = page.locator('tr', { hasText: 'friend1' })
  await friendRow.locator('button[role=switch]').click()
  await page.waitForSelector('tr:has-text("friend1") td:has-text("Disabled")')
  assert(true, 'user now shows Disabled status')

  console.log('10. re-enable the user')
  await friendRow.locator('button[role=switch]').click()
  await page.waitForSelector('tr:has-text("friend1") td:has-text("Active")')
  ...
  await page.waitForSelector('table td:has-text("friend1")')
  const adminNodeRows = await page.locator('tr', { hasText: 'friend1' }).count()
  assert(adminNodeRows === 2, `admin sees both of friend1's auto-created default nodes (got ${adminNodeRows})`)

  console.log('12. non-admin cannot see admin routes -- log out, log in as friend1')
  await logout(page)
  await login(page, 'friend1', 'friendpass123')
  ...
  console.log('14. friend1 has TWO pre-created default nodes ("player" + "maintenance"), both disabled, neither deletable')
  ...
  console.log('17. friend1 has no VPN region catalog yet -- log back in as admin to create one')
  await logout(page)
  ...
  console.log('19. friend1 picks the region from Settings -- it applies to all of their nodes, not per-node')
  await logout(page)
  await login(page, 'friend1', 'friendpass123')
  await page.waitForURL('**/nodes')
  ...
  assert(true, 'friend1 selected the us-east vpn region for their account')

  console.log('20. admin deletes the region -- friend1 falls back to no VPN, not an error')
  await logout(page)
  await login(page, 'admin', 'adminpass123')
  await page.waitForURL('**/users')
  await page.click('aside >> text=VPN')
  ...
  await page.waitForURL('**/users')
  await page.waitForSelector('table')
  const friendRow2 = page.locator('tr', { hasText: 'friend1' })
  assert(!(await friendRow2.locator('td:has-text("Never")').isVisible()), 'friend1 already logged in earlier in this run -- last login is a real timestamp, not "Never"')
  assert(await friendRow2.locator('td:has-text("0 / 2")').isVisible(), 'friend1 shows 2 total nodes, 0 active (they never enabled one)')
  await friendRow2.click()
  await page.waitForURL('**/admin/users/**')
  await page.waitForFunction(() => document.querySelector('h1')?.textContent === 'friend1')
  assert((await page.textContent('h1')) === 'friend1', 'user detail page header shows friend1')
  ...
  assert(await page.isVisible('text=friend1'), 'node detail shows the owner login, not a login/password form')
  assert(!(await page.isVisible('input[name=game_username]')), 'admin node detail has no editable game-login field -- read-only')
  await page.click('button:has-text("Back")')

  console.log('23. admin resets friend1\'s password from the detail page')
  await page.waitForURL('**/admin/users/**')
  await page.click('text=Reset password')
  await page.waitForSelector('[role=dialog]')
  await page.fill('[role=dialog] input[type=password]', 'newfriendpass456')
  await page.click('[role=dialog] button[type=submit]')
  await page.waitForSelector('[role=dialog]', { state: 'detached' })
  assert(true, 'password reset dialog closed after submit')

  console.log('24. disabling friend1 from the detail page stops their nodes (verified via node still shows Disabled, no error)')
  ...
  assert(true, 'friend1 now shows Disabled on their own detail page')
  await disableSwitch.click()
  await page.waitForSelector('text=Active')

  console.log('25. friend1 logs in with the reset password, and their display name / password settings work')
  await logout(page)
  await login(page, 'friend1', 'newfriendpass456')
  await page.waitForURL('**/nodes')
  assert(page.url().endsWith('/nodes'), 'friend1 logs in fine with the admin-reset password')
  ...
  await page.fill('#display-name', 'Friend One')
  await page.locator('form').first().locator('button[type=submit]').click()
  await page.waitForSelector('[data-sonner-toast]')
  assert(true, 'display name saved')

  console.log('26. friend1 changes their own password via Settings')
  const pwForm = page.locator('form').nth(1)
  await pwForm.locator('input[type=password]').nth(0).fill('newfriendpass456')
  await pwForm.locator('input[type=password]').nth(1).fill('finalfriendpass789')
  ...
  console.log('28. admin deletes friend1 -- their nodes go with them')
  await page.click('aside >> text=Users')
  await page.waitForURL('**/users')
  await page.click('tr:has-text("Friend One")')
  await page.waitForURL('**/admin/users/**')
  ...
  await page.waitForURL('**/users')
  await page.waitForSelector('table')
  assert(!(await page.isVisible('td:has-text("Friend One")')), 'friend1 no longer in the user list after delete')
